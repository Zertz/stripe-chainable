...
StripeChainable.prototype = {
  ...
  list: function (progress, callback) {
    ...
    const supportedObjects = ['charge', 'customer', 'plan', 'coupon', 'invoice', 'invoiceItem',
        'transfer', 'applicationFee', 'account', 'event', 'bitcoinReceiver', 'subscription',
        'fileUpload']
    ...
    const getObjects = (data) => {
      ...
      this._stripe[object + 's'].list(stripeOptions, stripeExtras, (err, objects) => {
        if (err) {
          this._reset()
          return callback(err)
        }
  ...
  listTransactions: function (progress, callback) {
    ...
    const supportedObjects = ['charge', 'refund', 'adjustment', 'applicationFee', 'applicationFeeRefund', 'transfer', 'transferFailure']
    const object = this._options.type
    ...
    const getObjects = (data) => {
      ...
      this._stripe.balance.listTransactions(stripeOptions, stripeExtras, (err, objects) => {
        if (err) {
          this._reset()
          return callback(err)
        }
